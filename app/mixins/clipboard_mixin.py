...
class ClipboardMixin:
    ...
    def _capture_clipboard_state(self) -> Dict[str, Any]:
        ...
        if is_MACOS and self._pending_macos_clipboard_restore_state:
            # macOS extension: if a paste-triggered restore is still pending, preserve
            # the original user clipboard content instead of the app's temporary text.
            logging.debug("Using pending macOS clipboard restore snapshot as the current clipboard baseline.")
            return self._pending_macos_clipboard_restore_state
        ...
        if is_MACOS:
            mime_snapshot = self._capture_macos_clipboard_state()
            if mime_snapshot is not None:
                state["mode"] = "macos_mime"
    ...
    def _restore_clipboard_state(self, state: Dict[str, Any]) -> None:
        ...
        if state.get("mode") == "macos_mime":
            if self._restore_macos_clipboard_state(state.get("mime_snapshot")):
                logging.debug("Clipboard content restored.")
                return
            logging.warning("Falling back to plain-text clipboard restoration on macOS.")
    ...
    def _capture_macos_clipboard_state(self) -> Optional[Dict[str, Any]]:
        """
        MacOS extension: capture the full clipboard MIME payload instead of only plain text.
        This preserves rich clipboard formats that would otherwise be lost after pasting.
        """
        if not is_MACOS:
            return None
        ...
        for mime_format in mime_data.formats():
            try:
                formats_snapshot.append({
                    "format": mime_format,
                    "data": bytes(mime_data.data(mime_format)),
                })
            except Exception as e:
                logging.warning(f"Could not snapshot macOS clipboard format '{mime_format}': {e}")

        return {"formats": formats_snapshot}

    def _restore_macos_clipboard_state(self, snapshot: Any) -> bool:
        """
        MacOS extension: restore a previously captured full clipboard MIME snapshot.
        Falls back to text-only restoration if any part of the native MIME restore fails.
        """
        if not is_MACOS or not isinstance(snapshot, dict):
            return False
        ...
        try:
            ...
            logging.warning(f"Failed to restore macOS clipboard MIME snapshot: {e}")
            return False

    def _schedule_macos_clipboard_restore(self, state: Dict[str, Any], delay_ms: int = 350) -> None:
        """
        MacOS extension: delay clipboard restoration until after the target app has
        consumed the paste event. Restoring too early can cause Cmd+V to insert nothing.
        """
        if not is_MACOS:
            self._restore_clipboard_state(state)
            return

        self._pending_macos_clipboard_restore_state = state
        self._macos_clipboard_restore_timer.stop()
        self._macos_clipboard_restore_timer.start(max(0, delay_ms))
        logging.debug("Scheduled macOS clipboard restore %sms after paste.", delay_ms)

    def _perform_macos_clipboard_restore(self) -> None:
        """Restore the delayed macOS clipboard snapshot after the paste has settled."""
        state = self._pending_macos_clipboard_restore_state
        self._pending_macos_clipboard_restore_state = None
        if not state:
            return
        try:
            self._restore_clipboard_state(state)
    ...
    def insert_transcribed_text(self, text: str) -> None:
        ...
        try:
            ...
            if restore:
                if is_MACOS:
                    self._schedule_macos_clipboard_restore(old_clipboard_state)
                else:
                    self._restore_clipboard_state(old_clipboard_state)
